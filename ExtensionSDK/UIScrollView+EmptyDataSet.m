...
{
    SEL lj_SEL = @selector(verticalOffset);
    objc_setAssociatedObject(self, lj_SEL, @(verticalOffset), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
-(void)setEmptyButtonClickBlock:(void (^)())emptyButtonClickBlock
{
    objc_setAssociatedObject(self, @selector(setEmptyButtonClickBlock:), emptyButtonClickBlock, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

-(void (^)())emptyButtonClickBlock
{
    return objc_getAssociatedObject(self, @selector(setEmptyButtonClickBlock:));
}

-(void)setEmptyDataSetWillAppearBlock:(void (^)())emptyDataSetWillAppearBlock
{
    objc_setAssociatedObject(self, @selector(setEmptyDataSetWillAppearBlock:), emptyDataSetWillAppearBlock, OBJC_ASSOCIATION_COPY_NONATOMIC);
}

-(void (^)())emptyDataSetWillAppearBlock
{
    return objc_getAssociatedObject(self, @selector(setEmptyDataSetWillAppearBlock:));
}

-(void)setEmptyButtonImage:(UIImage *)emptyButtonImage
{
    objc_setAssociatedObject(self, @selector(setEmptyButtonImage:), emptyButtonImage, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

-(UIImage *)emptyButtonImage
{
    return objc_getAssociatedObject(self, @selector(setEmptyButtonImage:));
}

-(void)setEmptyImage:(UIImage *)emptyImage
{
    objc_setAssociatedObject(self, @selector(setEmptyImage:), emptyImage, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

-(UIImage *)emptyImage
{
    UIImage * image = objc_getAssociatedObject(self, @selector(setEmptyImage:));
    if (!image) {
        image = [UIImage imageNamed:@"norecord"];
...
- (void)setEmptyTitle:(NSAttributedString *)emptyTitle {
    objc_setAssociatedObject(self, @selector(setEmptyTitle:), emptyTitle, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSAttributedString *)emptyTitle {
    NSAttributedString * title = objc_getAssociatedObject(self, @selector(setEmptyTitle:));
    if (!title) {
        title = [[NSAttributedString alloc]initWithString:@"暂无数据" attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15.0],NSForegroundColorAttributeName:[UIColor colorWithHexString:@"#999999"]}];
